...
void updateAnalogClock() {
  ...
  if (seconds != prevSecond) {
    // Instead of redrawing the entire ring, just update the segment that changed
    
    if (seconds == 0) {
      // Special case: when seconds reset to 0, clear the entire ring
      for (int i = 0; i < 60; i++) {
        int startAngle = 270 + (i * 6);
        int endAngle = startAngle + 6;
        drawSecondsArc(screenCenterX, screenCenterY, startAngle, endAngle, RING_RADIUS, RING_THICKNESS, TFT_BLACK);
      }
    } else if (prevSecond == 59 && seconds == 0) {
      // Another special case: when wrapping from 59 to 0, clear everything
      for (int i = 0; i < 60; i++) {
        int startAngle = 270 + (i * 6);
        int endAngle = startAngle + 6;
        drawSecondsArc(screenCenterX, screenCenterY, startAngle, endAngle, RING_RADIUS, RING_THICKNESS, TFT_BLACK);
    ...
    // If we're at the beginning of a new minute, need to redraw the ring
    if (seconds == 0) {
      // Draw the entire ring from scratch - segment by segment
      for (int i = 0; i < seconds; i++) {
        int startAngle = 270 + (i * 6);
        int endAngle = startAngle + 6;
        drawSecondsArc(screenCenterX, screenCenterY, startAngle, endAngle, RING_RADIUS, RING_THICKNESS, SECOND_RING_COLOR);
